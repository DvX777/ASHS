...
export const dashboardApiRoutes = new Elysia({ prefix: "/0x/api" })
  ...
  .get("/healer/scan", ({ request, set }: any) => {
    ...
    return { issues: [
      ...
      { type: "stuck-downloading",   label: "Stuck Downloading",        description: "Downloading but no active queue job for 30+ min",                  fix_preview: "Mark ready",       count: q("SELECT COUNT(*) as c FROM media WHERE status='downloading' AND EXISTS (SELECT 1 FROM media_files WHERE media_id=media.id AND status='complete')")  },
  ...
  .post("/healer/fix", ({ request, body, set }: any) => {
    ...
    const fix = (t: string) => {
      ...
      if (t === "stuck-downloading")   fixed += db.prepare("UPDATE media SET status='ready', updated_at=datetime('now') WHERE status NOT IN ('ready','removed') AND id IN (SELECT DISTINCT media_id FROM media_files WHERE status='complete')").run().changes;
